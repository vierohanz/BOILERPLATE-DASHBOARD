...
const SidebarLink = ({ icon, title, path, end, isCollapsed }: SidebarLinkProps) => (
  <NavLink to={path} end={end} className="block outline-none relative group/link">
    {({ isActive }) => (
      <>
        <div
          className={`flex items-center relative ${isCollapsed ? 'justify-center h-12 w-12 mx-auto' : 'w-full gap-4 px-6 h-12'}`}
        >
          <span
            className={`${isActive ? 'text-primary' : 'text-text-muted/60 group-hover/link:text-text-main'} transition-colors duration-300 shrink-0`}
          >
            {icon}
          </span>
          {!isCollapsed && (
            <span
              className={`text-[11px] font-bold uppercase tracking-wide whitespace-nowrap transition-colors duration-300 ${isActive ? 'text-primary' : 'text-text-muted group-hover/link:text-text-main'}`}
            >
              {title}
            </span>
          )}
...
const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
  const menuGroups = [
    {
      title: 'Main',
      items: [{ title: 'Users', icon: <Users size={20} />, path: '/dashboard/users' }],
    },
  ];
