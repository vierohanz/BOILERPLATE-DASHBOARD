...
export const Select: React.FC<SelectProps> = ({
          ...
          >
            {options.length === 0 ? (
              <div className="px-4 py-8 text-center">
                <p className="text-[10px] text-text-muted/30 font-black uppercase tracking-[0.2em]">
                  Tidak ada opsi
                </p>
              </div>
            ) : (
  ...
  return (
    <div
      className={`flex flex-col gap-2.5 ${className} ${forceTheme === 'light' ? 'light' : forceTheme === 'dark' ? 'dark' : ''}`}
      ref={containerRef}
    >
      {label && (
      ...
      <div className="relative group/select">
        ...
        >
          <div
            className={`absolute inset-y-0 left-0 pl-4.5 flex items-center pointer-events-none transition-all duration-300 ${
              forceTheme === 'light'
                ? '!text-slate-400 group-hover/select:!text-primary'
                : 'text-text-muted/30 group-hover/select:text-primary'
            }`}
          >
          ...
          <span
            className={`whitespace-nowrap ${selectedOption ? (forceTheme === 'light' ? '!text-slate-900' : 'text-text-main') : 'text-text-muted/20'}`}
          >
