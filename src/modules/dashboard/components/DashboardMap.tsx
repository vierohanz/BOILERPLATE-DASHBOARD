...
const DashboardMap: React.FC = () => {
  ...
  const mapStyle = (feature: any) => {
    ...
    const isTarget =
      hoveredData?.name === feature.properties.ADM2_EN ||
      selectedRegion?.properties?.ADM2_EN === feature.properties.ADM2_EN;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-text-main uppercase tracking-widest">
            Data Stunting & Persebaran
          </h2>
          <p className="text-text-muted text-xs mt-1">
            Peta sebaran stunting di wilayah Jawa Tengah
          </p>
        </div>
      </div>
      ...
      {/* Map */}
      <div
        className="bg-surface-card border border-border-subtle rounded-3xl overflow-hidden relative shadow-lg transition-colors duration-300"
        style={{ height: 520 }}
      >
        {geoData ? (
          <MapContainer
              ...
              >
                <div className="p-1 min-w-[180px]">
                  <p className="font-black text-sm uppercase tracking-wide">
                    {selectedRegion.properties.ADM2_EN}
                  </p>
                          ...
                          color: getColor(
                            parseFloat(
                              selectedRegion.properties[
                                `stunting_${selectedYear}prevalensi_stunting`
                              ]
                            ) || 0
                          ),
                        }}
                      >
                        {selectedRegion.properties[`stunting_${selectedYear}prevalensi_stunting`] ||
                          '0'}
                        %
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 pt-1">
                      {parseFloat(
                        ...
                        <>
                          ...
                          <span className="text-[10px] text-emerald-500 font-bold">Terkendali</span>
                        </>
                      )}
                    </div>
                  </div>
          ...
          {(hoveredData || selectedRegion) && (
            ...
            >
              ...
              <p className="text-sm font-black text-text-main leading-tight">
                {hoveredData
                  ? hoveredData.name?.split(' ').pop()
                  : selectedRegion?.properties.ADM2_EN?.split(' ').pop()}
              </p>
              <p
