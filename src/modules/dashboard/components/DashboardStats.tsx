      ...
      {stats.map((stat, i) => (
        ...
        >
          <div className="flex justify-between items-start mb-6 relative z-10">
            <div>
              <p className="text-text-muted text-[10px] font-black uppercase tracking-[0.2em] mb-3">{stat.label}</p>
              <h3 className="text-4xl font-black text-text-main tracking-tight">{stat.value}</h3>
            </div>
            <div 
              className="p-3.5 rounded-2xl bg-slate-50 dark:bg-white/5 text-slate-400 group-hover:text-primary transition-colors border border-border-subtle"
            >
              {stat.icon}
            </div>
          </div>
